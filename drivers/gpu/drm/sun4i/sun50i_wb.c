...
static void sun50i_wb_set_scaler(struct sun50i_wb *wb, u32 in_w, u32 in_h,
				 u32 out_w, u32 out_h, bool yuv)
{
	unsigned int i;
	const u32 *coeff;
	u32 control = yuv ? WB_CSC_EN : 0;
	u32 step_h;
	u32 step_v;
	u32 mid_h = in_h;
	u32 mid_w = in_w;
	...
	if (in_w > 2 * out_w) {
		writel(in_w | (out_w << 17), wb->regs + WB_CS_HORZ);
		mid_w = 2 * out_w;
		control |= WB_COARSE_EN;
	} else {
		writel(0, wb->regs + WB_CS_HORZ);
	}

	if (in_h > 2 * out_h) {
		writel(in_h | (out_h << 17), wb->regs + WB_CS_VERT);
		mid_h = 2 * out_h;
		control |= WB_COARSE_EN;
	} else {
		writel(0, wb->regs + WB_CS_VERT);
	}

	if (mid_w != out_w || mid_h != out_h) {
		control |= WB_FINE_EN;
		step_h = div_u64((u64)mid_w << 18, out_w) << 2;
		step_v = div_u64((u64)mid_h << 18, out_h) << 2;
		coeff = (mid_w > out_w || mid_h > out_h) ?
			sun50i_wb_coeff_down : sun50i_wb_coeff_up;
		for (i = 0; i < ARRAY_SIZE(sun50i_wb_coeff_up); i++) {
			writel(coeff[i], wb->regs + WB_YHCOEFF(i));
			writel(coeff[i], wb->regs + WB_CHCOEFF(i));
		}
	} else {
		step_h = 1 << 20;
		step_v = 1 << 20;
	...
	writel((mid_h - 1) << 16 | (mid_w - 1), wb->regs + WB_FS_INSIZE);
	writel((out_h - 1) << 16 | (out_w - 1), wb->regs + WB_FS_OUTSIZE);
	writel(step_h, wb->regs + WB_FS_HSTEP);
	writel(step_v, wb->regs + WB_FS_VSTEP);
	writel(control, wb->regs + WB_BYPASS);
}
...
{
	struct sun50i_wb *wb = conn_to_wb(conn_state->connector);
	struct drm_framebuffer *fb;

	if (!conn_state->writeback_job)
		return 0;
	if (!completion_done(&wb->job_done))
		return -EBUSY;
...
static void sun50i_wb_abort_job(struct drm_connector_state *conn_state,
				int status)
{
				    ...
				    struct drm_atomic_commit *state)
{
	...
	if (!wait_for_completion_timeout(&wb->job_done, 10 * HZ)) {
		drm_err(conn->dev, "timed out waiting for previous writeback job\n");
		sun50i_wb_abort_job(conn_state, -ETIMEDOUT);
		return;
	}
	...
	gctrl = WB_GCTRL_CLK_GATE | WB_GCTRL_AUTO_GATE | WB_GCTRL_PORT(port);
	writel(gctrl, wb->regs + WB_GCTRL);

	writel((h - 1) << 16 | (w - 1), wb->regs + WB_SIZE);
	writel(0, wb->regs + WB_CROP_COORD);
	writel((h - 1) << 16 | (w - 1), wb->regs + WB_CROP_SIZE);

	writel(lower_32_bits(addr[0]), wb->regs + WB_A_CH0_ADDR);
	writel(lower_32_bits(addr[1]), wb->regs + WB_A_CH1_ADDR);
	writel(lower_32_bits(addr[2]), wb->regs + WB_A_CH2_ADDR);
	writel(high, wb->regs + WB_A_HIGH_ADDR);
	writel(fb->pitches[0], wb->regs + WB_CH0_PITCH);
	writel(fb->format->num_planes > 1 ? fb->pitches[1] : 0,
	       wb->regs + WB_CH12_PITCH);

	writel(0, wb->regs + WB_ADDR_SWITCH);	/* group A, no auto switch */
	writel(format, wb->regs + WB_FORMAT);
	writel(1, wb->regs + WB_INT);

	sun50i_wb_set_scaler(wb, w, h, out_w, out_h, fb->format->is_yuv);
...
static const struct of_device_id sun50i_wb_of_table[] = {
	{ .compatible = "allwinner,sun50i-h6-de3-wb" },
	{ }
};
