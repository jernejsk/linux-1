				    ...
				    struct drm_atomic_state *state)
{
	...
	if (wb->job_pending) {
		...
		drm_writeback_signal_completion(&wb->wb_conn, -EBUSY);
		return;
	}
	...
	writel(WB_GCTRL_CLK_GATE | WB_GCTRL_AUTO_GATE | WB_GCTRL_PORT(port),
	       wb->regs + WB_GCTRL);
	...
	writel(readl(wb->regs + WB_GCTRL) | WB_GCTRL_START,
	       wb->regs + WB_GCTRL);
}
