			 ...
			 void __iomem *reg_base)
{
	...
	layer->scaler_rdma = sun8i_rdma_add_unit(rdma, reg_base + base, 0x240,
...
void sun8i_ui_scaler_enable(struct sun8i_layer *layer, bool enable)
{
	...
	sun8i_rdma_write(layer->scaler_rdma,
			 SUN8I_SCALER_GSU_CTRL(base) - base, val);
}
			   ...
			   u32 hscale, u32 vscale, u32 hphase, u32 vphase)
{
	...
	sun8i_rdma_write(layer->scaler_rdma, SUN8I_SCALER_GSU_OUTSIZE(base) - base,
			 outsize);
	sun8i_rdma_write(layer->scaler_rdma, SUN8I_SCALER_GSU_INSIZE(base) - base,
			 insize);
	sun8i_rdma_write(layer->scaler_rdma, SUN8I_SCALER_GSU_HSTEP(base) - base,
			 hscale);
	sun8i_rdma_write(layer->scaler_rdma, SUN8I_SCALER_GSU_VSTEP(base) - base,
			 vscale);
	sun8i_rdma_write(layer->scaler_rdma, SUN8I_SCALER_GSU_HPHASE(base) - base,
			 hphase);
	sun8i_rdma_write(layer->scaler_rdma, SUN8I_SCALER_GSU_VPHASE(base) - base,
			 vphase);
	offset = sun8i_ui_scaler_coef_offset(hscale);
	sun8i_rdma_memcpy(layer->scaler_rdma,
			  SUN8I_SCALER_GSU_HCOEFF(base, 0) - base,
