...
static int mmc_pwrseq_simple_probe(struct platform_device *pdev)
{
	...
	if (ngpio == 1) {
		...
		/*
		 * The reset-gpio shim only understands #gpio-cells = <2>, so on
		 * controllers that use a wider GPIO specifier the lookup fails
		 * with -ENOENT even though it was optional. Fall back to driving
		 * the reset line directly instead of failing the probe.
		 */
