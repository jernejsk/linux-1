		     ...
		     size_t *response_len, u8 *response_ctx)
{
	...
	if (!wait_for_completion_timeout(&wifi->cmd_done,
		...
		/*
		 * A controller that still acknowledges bus transfers but has
		 * stopped answering commands looks alive to every other
		 * detector this driver has: it keeps beaconing while nothing
		 * that needs an answer works. Consecutive timeouts are the
		 * only account of that state, so they are what asks for the
		 * firmware to be recovered.
		 */
		if (++wifi->cmd_timeouts == UWE5622_WIFI_CMD_TIMEOUT_LIMIT) {
			dev_err(wifi->dev,
				"%u consecutive command timeouts, recovering\n",
				wifi->cmd_timeouts);
