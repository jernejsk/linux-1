...
static void rtw8723b_lck(struct rtw_dev *rtwdev)
{
	...
	if ((val_ctx & BIT_MASK_CTX_TYPE) != 0) {
		...
		rf_mode = rtw_read_rf(rtwdev, RF_PATH_A, RF_MODE, RFREG_MASK);
		rtw_write_rf(rtwdev, RF_PATH_A, RF_MODE, RFREG_MASK, (rf_mode & 0x8FFFF) | BIT(16));
	} else {
	...
	/*  Channel 10 LC calibration issue for 8723BS with 26 MHz xtal */
	if (rtw_hci_type(rtwdev) == RTW_HCI_TYPE_SDIO && rtwdev->efuse.package_type >= PACKAGE_TFBGA90)
		rtw_write_rf(rtwdev, RF_PATH_A, RF_CFGCH, RFREG_MASK, lc_cal);
	if ((val_ctx & BIT_MASK_CTX_TYPE) != 0) {
		rtw_write8(rtwdev, REG_CTX, val_ctx);
		rtw_write_rf(rtwdev, RF_PATH_A, RF_MODE, RFREG_MASK, rf_mode);
	} else {
...
static void rtw8723bs_efuse_parsing(struct rtw_efuse *efuse,
				    struct rtw8723b_efuse *map)
{
	ether_addr_copy(efuse->addr, map->s.mac_addr);

	switch (map->s.package_type) {
	case 4:
		efuse->package_type = PACKAGE_TFBGA79;
		break;
	case 5:
		efuse->package_type = PACKAGE_TFBGA90;
		break;
	case 6:
		efuse->package_type = PACKAGE_QFN68;
		break;
	case 7:
		efuse->package_type = PACKAGE_TFBGA80;
		break;
	default:
		efuse->package_type = PACKAGE_DEFAULT;
		break;
	}
}
...
static int rtw8723b_read_efuse(struct rtw_dev *rtwdev, u8 *log_map)
{
	...
	map = (struct rtw8723b_efuse *)log_map;
	...
	case RTW_HCI_TYPE_SDIO:
		rtw8723bs_efuse_parsing(efuse, map);
		break;
	default:
		/* unsupported now */
		return -ENOTSUPP;
...
static void rtw8723b_query_rx_desc(struct rtw_dev *rtwdev, u8 *rx_desc,
				   struct rtw_rx_pkt_stat *pkt_stat,
...
static void rtw8723b_spur_cal(struct rtw_dev *rtwdev, u8 channel)
{
	...
	notch = rtw8723b_check_spur_ov_thres(rtwdev, channel, SPUR_THRES);
...
static void rtw8723b_iqk_backup_regs(struct rtw_dev *rtwdev,
...
static void rtw8723b_iqk_restore_regs(struct rtw_dev *rtwdev,
...
static void rtw8723b_iqk_backup_path_ctrl(struct rtw_dev *rtwdev,
					  struct iqk_backup_regs *backup)
{
...
static void rtw8723b_iqk_config_path_ctrl(struct rtw_dev *rtwdev)
{
...
static void rtw8723b_iqk_restore_path_ctrl(struct rtw_dev *rtwdev,
					   const struct iqk_backup_regs *backup)
{
...
static void rtw8723b_iqk_backup_lte_path_gnt(struct rtw_dev *rtwdev,
					     struct iqk_backup_regs *backup)
{
...
static void rtw8723b_iqk_config_lte_path_gnt(struct rtw_dev *rtwdev)
{
...
static void rtw8723b_iqk_restore_lte_path_gnt(struct rtw_dev *rtwdev,
...
static const struct rtw_8723b_iqk_cfg iqk_tx_cfg[PATH_NR] = {
	[PATH_S1] = {
		.name = "S1",
...
static u8 rtw8723b_iqk_check_tx_failed(struct rtw_dev *rtwdev,
				       const struct rtw_8723b_iqk_cfg *iqk_cfg)
{
	s32 tx_x, tx_y;
	u32 tx_fail;
...
static void rtw8723b_iqk_one_shot(struct rtw_dev *rtwdev, bool tx,
				  const struct rtw_8723b_iqk_cfg *iqk_cfg)
{
	...
	rtw8723b_iqk_config_lte_path_gnt(rtwdev);
...
static void rtw8723b_iqk_txrx_path_post(struct rtw_dev *rtwdev,
					const struct rtw_8723b_iqk_cfg *iqk_cfg,
					const struct iqk_backup_regs *backup)
{
	rtw8723b_iqk_restore_lte_path_gnt(rtwdev, backup);
...
static u8 rtw8723b_iqk_tx_path(struct rtw_dev *rtwdev,
			       const struct rtw_8723b_iqk_cfg *iqk_cfg,
			       const struct iqk_backup_regs *backup)
{
	u8 status;
	...
	rtw8723b_iqk_one_shot(rtwdev, true, iqk_cfg);
	status = rtw8723b_iqk_check_tx_failed(rtwdev, iqk_cfg);

	rtw8723b_iqk_txrx_path_post(rtwdev, iqk_cfg, backup);

	return status;
}

static u8 rtw8723b_iqk_rx_path(struct rtw_dev *rtwdev,
			       const struct rtw_8723b_iqk_cfg *iqk_cfg,
			       const struct iqk_backup_regs *backup)
{
	u32 tx_x, tx_y;
	...
	rtw8723b_iqk_one_shot(rtwdev, false, iqk_cfg);
	status = rtw8723b_iqk_check_tx_failed(rtwdev, iqk_cfg);
	...
	rtw8723b_iqk_one_shot(rtwdev, false, iqk_cfg);
	status |= rtw8723b_iqk_check_rx_failed(rtwdev, iqk_cfg);
...
static
void rtw8723b_iqk_fill_s1_matrix(struct rtw_dev *rtwdev, const s32 result[])
{
	s32 oldval_1;
	s32 x, y;
	s32 tx1_a, tx1_a_ext;
...
static
void rtw8723b_iqk_fill_s0_matrix(struct rtw_dev *rtwdev, const s32 result[])
{
	s32 oldval_0;
	s32 x, y;
	s32 tx0_a, tx0_a_ext;
...
static void rtw8723b_iqk_path_adda_on(struct rtw_dev *rtwdev)
{
	int i;
...
static void rtw8723b_iqk_config_mac(struct rtw_dev *rtwdev)
{
	rtw_write8(rtwdev, REG_TXPAUSE, 0xff);
}

static
void rtw8723b_iqk_rf_standby(struct rtw_dev *rtwdev, enum rtw_rf_path path)
{
...
static
bool rtw8723b_iqk_similarity_cmp(struct rtw_dev *rtwdev, s32 result[][IQK_NR],
				 u8 c1, u8 c2)
{
	u32 i, j, diff;
	u32 bitmap = 0;
...
static
void rtw8723b_iqk_precfg_path(struct rtw_dev *rtwdev, enum rtw8723x_path path)
{
	if (path == PATH_S0) {
		rtw8723b_iqk_rf_standby(rtwdev, RF_PATH_A);
		rtw8723b_iqk_path_adda_on(rtwdev);
	}
	...
	if (path == PATH_S1) {
		rtw8723b_iqk_rf_standby(rtwdev, RF_PATH_B);
		rtw8723b_iqk_path_adda_on(rtwdev);
	}
}
			    ...
			    const struct iqk_backup_regs *backup)
{
	...
	rtw8723b_iqk_path_adda_on(rtwdev);
	rtw8723b_iqk_config_mac(rtwdev);
	...
	rtw8723b_iqk_precfg_path(rtwdev, PATH_S1);

	for (i = 0; i < PATH_IQK_RETRY; i++) {
		s1_ok = rtw8723b_iqk_tx_path(rtwdev, &iqk_tx_cfg[PATH_S1], backup);
		if (s1_ok == IQK_TX_OK) {
			rtw_dbg(rtwdev, RTW_DBG_RFK,
	...
	for (i = 0; i < PATH_IQK_RETRY; i++) {
		s1_ok = rtw8723b_iqk_rx_path(rtwdev, &iqk_tx_cfg[PATH_S1], backup);
	...
	rtw8723b_iqk_precfg_path(rtwdev, PATH_S0);

	for (i = 0; i < PATH_IQK_RETRY; i++) {
		s0_ok = rtw8723b_iqk_tx_path(rtwdev, &iqk_tx_cfg[PATH_S0], backup);
		if (s0_ok == IQK_TX_OK) {
			rtw_dbg(rtwdev, RTW_DBG_RFK,
	...
	for (i = 0; i < PATH_IQK_RETRY; i++) {
		s0_ok = rtw8723b_iqk_rx_path(rtwdev, &iqk_tx_cfg[PATH_S0], backup);
...
static void rtw8723b_phy_calibration(struct rtw_dev *rtwdev)
{
	...
	rtw8723b_iqk_backup_path_ctrl(rtwdev, &backup);
	rtw8723b_iqk_backup_lte_path_gnt(rtwdev, &backup);
	rtw8723b_iqk_backup_regs(rtwdev, &backup);

	for (i = IQK_ROUND_0; i <= IQK_ROUND_2; i++) {
		rtw8723b_iqk_config_path_ctrl(rtwdev);
		rtw8723b_iqk_config_lte_path_gnt(rtwdev);

		rtw8723b_iqk_one_round(rtwdev, result, i, &backup);

		if (i > IQK_ROUND_0)
			rtw8723b_iqk_restore_regs(rtwdev, &backup);
		rtw8723b_iqk_restore_lte_path_gnt(rtwdev, &backup);
		rtw8723b_iqk_restore_path_ctrl(rtwdev, &backup);

		for (j = IQK_ROUND_0; j < i; j++) {
			good = rtw8723b_iqk_similarity_cmp(rtwdev, result, j, i);
...
iqk_done:
	rtw8723b_iqk_fill_s1_matrix(rtwdev, result[final_candidate]);
	rtw8723b_iqk_fill_s0_matrix(rtwdev, result[final_candidate]);
...
static u8 rtw8723b_pwrtrack_get_limit_ofdm(struct rtw_dev *rtwdev)
{
	struct rtw_dm_info *dm_info = &rtwdev->dm_info;
...
static void rtw8723b_pwrtrack_set(struct rtw_dev *rtwdev, u8 path)
{
	...
	limit_ofdm = rtw8723b_pwrtrack_get_limit_ofdm(rtwdev);
...
static void rtw8723b_pwrtrack_set_xtal(struct rtw_dev *rtwdev, u8 therm_path,
				       u8 delta)
{
	struct rtw_dm_info *dm_info = &rtwdev->dm_info;
...
static void rtw8723b_phy_pwrtrack(struct rtw_dev *rtwdev)
{
	...
	rtw8723b_pwrtrack_set_xtal(rtwdev, RF_PATH_A, delta);
...
static struct rtw_chip_ops rtw8723b_ops = {
	.phy_set_param		= rtw8723b_phy_set_param,
	.read_efuse		= rtw8723b_read_efuse,
	.query_rx_desc		= rtw8723b_query_rx_desc,
...
static const struct rtw_pwr_seq_cmd *card_enable_flow_8723b[] = {
...
static const struct rtw_prioq_addrs prioq_addrs_8723b = {
...
static const struct rtw_hw_reg rtw8723b_dig[] = {
	[0] = { .addr = 0xc50, .mask = 0x7f },
	[1] = { .addr = 0xc50, .mask = 0x7f },
};

static const struct rtw_hw_reg rtw8723b_dig_cck[] = {
	[0] = { .addr = 0xa0c, .mask = 0x3f00 },
};

static const struct rtw_rf_sipi_addr rtw8723b_rf_sipi_addr[] = {
...
static const struct rtw_ltecoex_addr rtw8723b_ltecoex_addr = {
...
const struct rtw_chip_info rtw8723b_hw_spec = {
	...
	.prioq_addrs = &prioq_addrs_8723b,
	.intf_table = &phy_para_table_8723b,
	.dig = rtw8723b_dig,
	.dig_cck = rtw8723b_dig_cck,
	.rf_sipi_addr = {0x840, 0x844},
	.rf_sipi_read_addr = rtw8723b_rf_sipi_addr,
	.fix_rf_phy_num = 2,
	.ltecoex_addr = &rtw8723b_ltecoex_addr,
